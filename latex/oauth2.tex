...
\end{alltt}
I do not want to sped too much time on this one. It is for sure one of the fastest
...
Provide an access token at each client, which they could use to validate their identity at each API request.
...
\begin{enumerate}
    ...
    \item
        The server returns the access token with additional informations, such as expire date and JWT.
    \item
