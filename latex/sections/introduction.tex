...
\\
If you wanted to grant access to your private data to a third party application,
...
But things have changed, now (and even back then) there is no guarantee that an
application will keep your credentials all safe and sound, they could store them
in plain text, or even worse use them for malicious purposes, by accessing more
informations than the bare necessities.

To prevent that everyone finds a different solution to the same problem, we have
agreed to web-standards, to securely allow third party applications to access
protected resources.
\\
The standard protocol discussed in this paper takes the name of \textbf{OAuth2}.
...
It is worth noting that this is not the only protocol used by the industry (see
SAML\footnote{More on SAML: \url{https://en.wikipedia.org/wiki/Security_Assertion_Markup_Language}}).
